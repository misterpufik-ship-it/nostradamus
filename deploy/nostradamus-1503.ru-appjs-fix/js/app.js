...
const BALANCE_RESET_AT = new Date("2026-06-01T00:00:00+03:00");
...
async function loadRecords(dateKey) {
  const data = await requestJson(`${API_BASE}/records?date=${encodeURIComponent(dateKey)}`);
  return data.records || [];
}

async function loadAllRecords() {
  const data = await requestJson(`${API_BASE}/records`);
  return data.records || [];
}
...
function isClosedRecord(record) {
  return Boolean(record.arrivalTime && record.leaveTime);
}

function isAfterBalanceReset(record) {
  return new Date(record.arrivalTime) >= BALANCE_RESET_AT;
}
...
function calculateCumulativeBalance(records) {
  const workedByDate = records.filter(isClosedRecord).filter(isAfterBalanceReset).reduce((days, record) => {
    if (!days[record.date]) {
      days[record.date] = 0;
    }

    days[record.date] += calculateWorked(record);
    return days;
  }, {});

  const balance = Object.values(workedByDate).reduce((total, worked) => total + worked - WORKDAY_MS, 0);

  return {
    under: Math.max(0, -balance),
    over: Math.max(0, balance),
  };
}
...
async function render() {
  ...
  try {
    [visibleRecords, allRecords] = await Promise.all([loadRecords(selectedDate), loadAllRecords()]);
  } catch (error) {
  ...
  const cumulativeBalance = calculateCumulativeBalance(allRecords);

  workedToday.textContent = formatDuration(totals.worked);
  underTime.textContent = formatDuration(cumulativeBalance.under);
  overTime.textContent = formatDuration(cumulativeBalance.over);
...
arrivalBtn.addEventListener("click", async () => {
  ...
  try {
    await requestJson(`${API_BASE}/arrival`, {
      method: "POST",
      body: JSON.stringify({
...
leaveBtn.addEventListener("click", async () => {
  ...
  try {
    await requestJson(`${API_BASE}/leave`, {
      method: "POST",
      body: JSON.stringify({
        date: dateKey,
        employee,
...
clearDayBtn.addEventListener("click", async () => {
  ...
  try {
    await requestJson(`${API_BASE}/records?date=${encodeURIComponent(selectedDate)}`, { method: "DELETE" });
  } catch (error) {
    setStatus(error.message, "error");
    return;
  }
